...
var er = function () {
    ...
    ActionBase_ = {
		...
		addDataControl: function(){
			var me = this;
			return function(dataControl){
				me._dataControl = dataControl;
			}
		},
		
		
		clearDataControl: function(){
       ...
       /*     function repeatCallback() {
			...
			//串改并	
			for (var j = 0; j < len; j++) {
				initerMap[initerList[j]].call(me, repeatCallback);
			}
			//Context初始化的回调函数
	        function repeatCallback() {
				i++;
        ...
        setContext: function (key, value) {
			var me = this, 
				oldValue = me.getContext(key), 
				isQuotype = (typeof(value) == "object" || typeof(value) == "function"),
				dataControls, changeControls, ui,
				i, len, control, tmp;
			if (isQuotype || oldValue !== value) {
				context_.set(key, value, me._contextId);
				me._dataControl && (dataControls = me._dataControl[key]);
				if (dataControls) {
					changeControls = me._changeControl;
					for (i = 0, len = dataControls.length; i < len; i++) {
						tmp = dataControls[i].split(':');
						ui = tmp[0];
						if(typeof(changeControls[ui]) == "undefined"){
							changeControls[ui] = [];
						}
						changeControls[ui].push(tmp[1] + ':' + key);
					}
				}
			}
        },
        ...
        return {
            ...
            set: function (name, value, opt_contextId) {
                ...
                if (!context) {
                    ...
                    nirvana.util.logError('ER: private context "' + opt_contextId + '" is not exist.');
                    return;
                }
