 ...
 * 
 * path: accountOptimizer/all/widget/widget14.js 
 * desc: 计划优化
 * author: yangji01@baidu.com
 * date: $Date: 2011/11/25 $
 */

ao.widget14 = new er.Action({
    ...
    */
			...
			var me = this,
				...
				tabToButtonAndTip = {
					0 : {
						...
						tip : '%n个计划暂停推广，您可以在操作栏中点击启用恢复。'
					},
					1 : { //计划被删除
						button : [
						],
						tip : '以下计划昨日被删除，导致展现量突降，建议恢复已删除计划中优质的关键词，提升推广效果。'
					}
				},
				allButtonIds = [
					'modifyBid',
    				...
    				allData= {
						planinfo : {
							content: function(item){
								var func =  nirvana.aoWidgetRender.planinfo(),
									html = func(item),
	...
	onafterrender: function(){
		var me = this,
			...
			tabIndexToTarget = {
				0 : 'aowidget_batch_active_plan'
			},
			tabIndexToContent = {
				0 : {
					title : '启用推广计划',
					content : ['您确定启用所选的' , '个推广计划吗？']
				}
			},
			tabToItem = {
				0 : 'plan'
	...
	updateTableContent : function(tabIndex, callback){
		...
		switch (tabIndex) {
			case 0:
				fbs.ao.getPlanPause( nirvana.aoWidgetAction.requestTpl(me,params, startindex, endindex, 0, callback) );
				break;
			case 1 :
				fbs.aodecr.getPlanDeleteDetail( nirvana.aoWidgetAction.requestTpl(me,params, startindex, endindex, 1, callback) );				
				break;
			default :
				break;				
		}
...
//					er.controller.fireMain('reload', {});
					...
					ui.util.get('SideNav').refreshUnitList(idArr);
